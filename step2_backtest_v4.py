...
SMA_TREND = 99
SWING_LB  = 10
RETRACE   = 0.003   # 0.3% ถือว่า "แตะ" EMA30

TP1_R     = 1.2     # Order 1 RR
TP2_R     = 3.1     # Order 2 RR
MIN_SCORE = 8       # ต้องได้คะแนน >= 8 ถึงจะเข้า
...
def indicators(df):
    ...
    df["sess"] = pd.cut(df["hour"],
        bins=[-1,1,8,13,21,24],
        labels=["LATE","ASIA","EUROPE","US","LATE2"]).astype(str)
    df["sess"] = df["sess"].replace("LATE2","LATE")
...
def backtest(df, sym):
    ...
    for i in range(1, len(df)):
        ...
        if in_trade:
            hit_sl  = (side=="LONG" and r["low"]<=sl)  or (side=="SHORT" and r["high"]>=sl)
            hit_tp1 = (side=="LONG" and r["high"]>=tp1) or (side=="SHORT" and r["low"]<=tp1)
            hit_tp2 = (side=="LONG" and r["high"]>=tp2) or (side=="SHORT" and r["low"]<=tp2)

            if hit_sl:
                pnl = -10 if not tp1_hit else -10*0.5 + 10*0.5*TP1_R
                trades.append({**meta,"exit":r.name,"outcome":"LOSS","pnl":round(pnl,2),"tp1_hit":tp1_hit})
                in_trade = tp1_hit = False
            elif tp1_hit and hit_tp2:
                pnl = 10*0.5*TP1_R + 10*0.5*TP2_R
                trades.append({**meta,"exit":r.name,"outcome":"WIN","pnl":round(pnl,2),"tp1_hit":True})
                in_trade = tp1_hit = False
            elif hit_tp1:
                tp1_hit = True
        else:
            ...
            ep_p = r["open"]
            atr  = p["atr"]
            sl_p = min(p["sl"], ep_p - atr) if chosen=="LONG" else max(p["sh"], ep_p + atr)
            dist = abs(ep_p - sl_p)
            if dist < ep_p * 0.001:
                continue
...
def metrics(t):
    ...
    sess    = t.groupby("sess").agg(n=("outcome","count"),w=("outcome",lambda x:(x=="WIN").sum()),p=("pnl","mean"))
...
def main():
    print("="*54)
    print("  AI TRADE SYSTEM — BACKTEST v4")
    print("  EMA7/30 · SMA99 · BOS/CHoCH/QM · RR1:1.2+1:3.1")
    print("="*54)
    ...
    for sym in SYMBOLS:
        ...
        d4h = load(sym, TF_4H)
        if d1h.empty:
            print("  ไม่มีข้อมูล — รัน step1 ก่อน")
            continue
        ...
        if t.empty:
            ...
            continue
        all_trades.append(t)
        report(metrics(t), f"{sym}  [{len(t)} trades]")
    ...
    combined.to_csv("backtest_results_v4.csv", index=False)
    print(f"\n[SAVE] {len(combined):,} trades → backtest_results_v4.csv")
